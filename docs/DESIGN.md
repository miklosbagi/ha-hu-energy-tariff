...
## Backfill: reuse the live engine as a replay, don't build a second one

A fresh setup has no way to know what was really consumed before it started watching - `coordinator.py::async_setup` baselines at whatever the source sensor currently reads and starts the quota/fee proration clock at that instant (`PricingPeriod.valid_from`). Installing partway through a tariff year means the app under-tracks relative to a real DSO bill: it has no idea real quota was already used in the months before setup.

`backfill.py` closes part of that gap using data that's often already sitting in the user's own Home Assistant instance: `homeassistant.components.recorder.statistics.statistics_during_period` (the same internal API the Energy Dashboard itself uses), which keeps daily-resolution history indefinitely for any `state_class: total`/`total_increasing` sensor - already a requirement for the source sensor. The module is split in two, deliberately:

- `async_fetch_daily_deltas` is the only HA-coupled half - it degrades to `None`, never raises, whenever there's nothing to backfill from (Recorder not loaded, or no statistics for this entity in the window). Both are normal, expected situations, not error states.
- `replay_deltas` is a plain function with no `hass` dependency, and **no new pricing logic** - it feeds historical per-day deltas through the exact same `TariffStrategy.calculate()` used for live events, one simulated day at a time, threading state through. A backfilled day is priced/quota-split identically to how it would have been if the integration had been running that whole time, because it *is* the same code path, just called in a loop instead of from a state-change event. `mvm_a1.py` has zero backfill-specific code.

**The clamp dependency this relies on**: `_eligible_quota_kwh` and `_fixed_fee_accrual_start` both clamp their day-count to `PricingPeriod.valid_from` (see the section above). Backfill only works because `config_flow.py` backdates `valid_from` to the actual backfill start date when the user accepts the offer - if it stayed at "now" (today's default), those existing clamps would silently cancel the whole backfill out, since both would still only count days since setup. This is the same clamp, reused for its original purpose (`_fixed_fee_accrual_start`'s docstring), not a special case bolted on for backfill.

**Deliberate scope limit**: the backfill window is capped at the *later* of the current tariff year's start (1 Aug) or 1 January of the current calendar year (`backfill_start_date`) - never further back, even if more Recorder history exists. Installing Jan-Jul only reaches back to that Jan 1; installing Aug-Dec reaches back to that Aug 1 (Jan 1 hasn't happened yet in the current tariff year, so it doesn't bind). This caps how far backfill ever reaches at roughly 12 months using a boundary any user immediately understands, while never crossing into a different tariff year's quota cycle.

**Explicitly out of scope for this pass**: backfilling from an entity *other* than the one being tracked live (e.g. a long-running P1 smart-meter sensor's history, seeding a freshly-added inverter's own grid-import sensor) - a real, separate scenario, flagged for a future pass rather than folded in here. Re-triggering backfill later via the options flow, or backfilling into an already-running coordinator's accumulated state, is also out of scope - only ever offered once, at initial setup, when there's no persisted state yet to reconcile against.
...
1. **Headless engine tests** (`tests/unit/test_quota_*.py`, `test_tariff_year_*.py`, `test_fixed_monthly_fee.py`, `test_unit_normalization.py`, `test_price_validity_boundary.py`, `test_backfill.py`) call `A1Strategy.calculate()` and pure model functions directly, with hand-built fixtures - no Home Assistant import beyond `homeassistant.core.State`/`Enum` types that don't require a running instance. This is what the spec asks for explicitly ("tariff-engine tests should be possible without booting the complete Home Assistant entity layer wherever practical") and it's what keeps the core quota/proration/boundary-split math trivially reviewable in isolation. `test_backfill.py::TestReplayDeltas` follows the same pattern for the backfill replay loop - it's plain-function logic, not HA-coupled, so it belongs here too.
2. **HA-boot integration tests** (`test_config_flow.py`, `test_integration_setup.py`, `test_sensor_descriptions.py`, `test_backfill_recorder.py`) use `pytest-homeassistant-custom-component`'s lightweight test `hass` fixture to drive the real config flow, `async_setup_entry`, live state-change handling, and entity creation - proving the *wiring*, not just the math. This layer is what caught two real bugs during development that the headless tests structurally couldn't: A1 missing from the tariff-plan catalog (so it was unselectable), and the device name leaking an internal id into the user-visible entity names. `test_backfill_recorder.py` mocks `statistics_during_period`'s return value rather than seeding a real Recorder database - it's testing this integration's own parsing of that API's output, not re-deriving HA core's own statistics-bucketing algorithm.
