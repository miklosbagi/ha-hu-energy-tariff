...
Two more Gitea Actions workflows bridge the two platforms, with deliberately different levels of automation depending on which direction is riskier:

- **`promote-to-github.yml`** (Gitea → GitHub, on every Gitea PR merge to `main`): pushes that merge as a new GitHub branch (`gitea-promote/pr-<N>`) and opens a matching GitHub PR - same title/body, plus the originating PR's `Tag/*` label copied across (`.github/workflows/label-pr.yml` skips its own default-`Tag/Patch` add for these branches specifically, so it doesn't end up stacked next to the copied one). **It only ever opens a PR - it never pushes to GitHub `main` directly and never merges anything itself.** This is the one direction that touches the public, canonical repo, so a human always reviews and merges it there, same as any other GitHub PR - which is what actually triggers the release, per the section above.
- **`sync-from-github.yml`** (GitHub → Gitea, scheduled every 30 minutes plus manual trigger): whenever GitHub `main` is ahead of Gitea `main` - typically a community PR merged directly on GitHub - opens a Gitea PR from GitHub's current tip and merges it via Gitea's `fast-forward-only` style once required checks pass (`merge_when_checks_succeed`), fully automated. Safe to leave unattended because a fast-forward can never rewrite history or discard anything; Gitea is also infrastructure already fully trusted with push access for other automation. If the two histories have genuinely diverged (both sides have commits the other lacks - shouldn't normally happen, but did once during this restructuring, see the repo's git history for that reconciliation), the job fails loudly instead of guessing at a resolution.
